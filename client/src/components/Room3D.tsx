...
function StarField() {
  ...
  const { positions, colors } = (() => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Slightly blue-white tinted stars
  ...
  // Gentle slow rotation of starfield
  useFrame((_, delta) => {
    if (starsRef.current) {
  ...
  return (
    <mesh scale={[-1, 1, 1]}>
      ...
      <meshBasicMaterial side={THREE.BackSide}>
        <primitive
          object={(() => {
            // Procedural deep space gradient texture
            ...
            addNebula(200, 150, 200, "rgb(20, 40, 120)", 0.4);
            addNebula(700, 100, 180, "rgb(10, 30, 100)", 0.35);
            addNebula(500, 300, 250, "rgb(15, 20, 80)", 0.3);
            addNebula(850, 350, 160, "rgb(30, 15, 80)", 0.2);
            addNebula(100, 350, 140, "rgb(10, 50, 100)", 0.25);

            // Saturn-like planet (right side like the reference image)
...
// ─── Animated RGB lights behind the computer ─────────────────────
function RGBLights() {
  ...
  // Position lights behind/around the PC desk area
  // The desk is around x: 0.5, y: -0.2, z: -0.5 after room transform
  return (
    <>
      {/* Behind monitor - main RGB glow */}
      <pointLight
        ref={light1}
