...
function SkyDome() {
  const texture = useMemo(() => {
    ...
    for (let i = 0; i < 2500; i++) {
      ...
      const br = 170 + seed(i * 4.1) * 85, al = 0.45 + seed(i * 6.2) * 0.45;
      drawStar(sx, sy, 0.5 + seed(i * 2.1) * 0.4, br, br, Math.min(255, br + 25), al);
    }
    for (let i = 0; i < 500; i++) {
      const sx = seed(i * 5.7 + 100) * W, sy = seed(i * 2.9 + 100) * H;
      const br = 200 + seed(i * 8.3) * 55, al = 0.65 + seed(i * 3.4) * 0.35;
      drawStar(sx, sy, 0.9 + seed(i * 4.1) * 0.6, Math.max(180, br - 15), Math.max(185, br - 10), br, al);
    }
    for (let i = 0; i < 80; i++) {
      const sx = seed(i * 9.1 + 200) * W, sy = seed(i * 4.7 + 200) * H;
      const br = 230 + seed(i * 6.1) * 25, rad = 1.4 + seed(i * 3.3) * 0.6;
    ...
    satAtm.addColorStop(0, "rgba(100,130,220,0.3)"); satAtm.addColorStop(1, "rgba(0,0,0,0)");
    ...
    .forEach(({y,h,a}) => { ctx.fillStyle=`rgba(80,100,180,${a})`; ctx.fillRect(px-pr, py+y*pr, pr*2, h*pr); });
    ctx.restore();
    ...
    matm.addColorStop(0,"rgba(180,60,25,0.22)"); matm.addColorStop(1,"rgba(0,0,0,0)");
...
// ─── Room model WITH RGB lights attached directly to monitor mesh ──
function RoomModel() {
  ...
  useFrame((_, dt) => {
    t.current += dt * 0.45;
    const s = t.current;
    light1.current?.color.setHSL((s * 0.06) % 1, 1, 0.5);
    light2.current?.color.setHSL(((s * 0.06) + 0.33) % 1, 1, 0.5);
    light3.current?.color.setHSL(((s * 0.06) + 0.66) % 1, 1, 0.5);
  });
  ...
  useEffect(() => {
    scene.traverse((child) => {
      ...
      // Attach lights directly to monitor mesh — they inherit its world transform
      if (mesh.name.toLowerCase().includes("cube.049") || mesh.name.toLowerCase().includes("cube049")) {
        if (light1.current && !mesh.children.includes(light1.current)) {
          // In monitor's LOCAL space: 0,0,0 = monitor centre
          // We want BEHIND the screen = positive Z in monitor local (away from viewer)
          // and slightly left = negative X
          light1.current.position.set(-0.3,  0.1,  0.4); // behind screen left
          light2.current!.position.set(-0.1,  0.3,  0.5); // behind screen upper
          light3.current!.position.set(-0.2, -0.1,  0.3); // behind screen lower
          mesh.add(light1.current);
          mesh.add(light2.current!);
          mesh.add(light3.current!);
        }
      }
  ...
  return (
    <>
      ...
      {/* Lights declared here so refs exist before useEffect runs */}
      <pointLight ref={light1} intensity={5}   distance={1.5} decay={3} color="#ff00aa"/>
      <pointLight ref={light2} intensity={4}   distance={1.3} decay={3} color="#00aaff"/>
      <pointLight ref={light3} intensity={3.5} distance={1.2} decay={3} color="#aa00ff"/>
    </>
  );
}
