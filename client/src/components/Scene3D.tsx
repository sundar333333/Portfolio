...
interface Scene3DProps {
  ...
  onLogoSequenceProgress?: (progress: number) => void;
}
...
function ScrollSceneContent({ hoveredText, onTVClick, isVideoPlaying, onWorkSectionChange, onScrollProgress, onWhiteSectionProgress, onCircleProgress, onLogoSequenceProgress }: ScrollSceneProps) {
  ...
  const circleStart = 0.90;
  const circleEnd = 0.92;
  const logoSequenceStart = 0.92;
  ...
  useFrame(() => {
    ...
    if (offset > circleStart) {
      const circleProgress = (offset - circleStart) / (circleEnd - circleStart);
      onCircleProgress?.(Math.min(1, circleProgress));
    } else {
      onCircleProgress?.(0);
    }

    if (offset > logoSequenceStart) {
      const logoProgress = (offset - logoSequenceStart) / (1 - logoSequenceStart);
      onLogoSequenceProgress?.(Math.min(1, logoProgress));
    } else {
      onLogoSequenceProgress?.(0);
    }
  });
...
export function Scene3D({ hoveredText, onTVClick, isVideoPlaying, onWorkSectionChange, onScrollProgress, onWhiteSectionProgress, onCircleProgress, onLogoSequenceProgress }: Scene3DProps) {
  return (
        ...
        <Suspense fallback={null}>
          <ScrollControls pages={22} damping={0.2}>
            <ScrollSceneContent
              ...
              onLogoSequenceProgress={onLogoSequenceProgress}
            />
          </ScrollControls>
        </Suspense>
      </Canvas>
